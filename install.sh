...
TARGFILE=$(curl -s $(curl -s https://github.com/$RELEASES|grep  expanded_assets|sed 's/.\+src="//g'|cut -d '"' -f1)|grep /releases/|cut -d'"' -f2|grep MYARCH)
echo "getting $TARGFILE FROM $RELEASES"
